...
write_csv(x = kruskal_test, file = "4e_kruskal_test.csv")

#Since p-value from Kruskal-Wallis test is <0.05, it means that some of the pairwaise
# group comparisons would be positive. For pairwaise comparison we will do
...
significance_levels <- dunn_test_data |>
  ...
  rows_update(tibble(group1 = "RBD4059 9mg/kg", group2 = "PBS", x_min = 1,
                     x_max = 4, y = 43))
                                                                                            
#Write Dunn test data as .csv file
write_csv(x = dunn_test_data, file = "4e_dunn_test.csv")

#Change factor levels for plotting preference
auc_data$sample <- factor(auc_data$sample,levels = c("PBS", "RBD4059 1mg/kg",
                                                     "RBD4059 3mg/kg",
                                                     "RBD4059 9mg/kg",
                                                     "Enoxaparine"))
...
plot_auc <- auc_data |>
  ...
  geom_boxplot(aes(fill = sample), width = 0.5, linewidth = 0.15, show.legend = F) +
  theme_classic() +
...
#Save the plot
ggsave("4e_plot_auc.pdf", plot = plot_auc, scale = 1, width = 6, height = 6,
       units = "cm", dpi = 300)
...
data_timecourse_plotting <- raw_data_timecourse |>
  ...
  summarise(mean = mean(blood_flow_velocity, na.rm = T), sd = sd(blood_flow_velocity, na.rm = T)) |>
  mutate(upper = mean + sd,
         lower = mean - sd)

#Change factor levels of sample for timecourse plotting preference
data_timecourse_plotting$sample <- factor(data_timecourse_plotting$sample, levels = c("PBS", "RBD4059 1mg/kg",
                                                                                      "RBD4059 3mg/kg",
                                                                                      "RBD4059 9mg/kg",
                                                                                      "Enoxaparine"))
...
ggsave("4d_plot.pdf", plot = plot_timecourse, scale = 1, width = 15,
...
write_csv(x = anova_timecourse, file = "4d_anova_timecourse.csv")

#Since the two-way anova test showed a p-value <0.05 for sample and time, 
#we will perform Dunnett multiple comparison test for post-hoc analysis.
...
write.xlsx(x = dunnett_timecourse_data_pbs, file = "4d_dunnett.xlsx",
           sheetName = "control = PBS")
write.xlsx(x = dunnett_timecourse_data_enoxaparine, file = "4d_dunnett.xlsx",
